<script>
    ...
    $: changesCount = Object.entries($changes).reduce((acc, [collectionId, collectionChanges]) => {
        if ($config[collectionId] && $config[collectionId].hasOwnProperty('isCollection') && $config[collectionId].isCollection)
            return acc + Object.keys(collectionChanges).length;
        else
            return acc + 1;
    }, 0);
                ...
                {#each Object.entries($changes) as [collectionId, collectionChanges]}
                    {#if $config[collectionId] && $config[collectionId].hasOwnProperty('isCollection') && $config[collectionId].isCollection}
                        {#each Object.entries(collectionChanges) as [recordId, change]}
                            <div class="card">
                                <div>
                                    <h4><span>{$config[collectionId].title}:</span> {change.title}</h4>
                                </div>
    ...
    <div class="header-actions">
        {#if $changes && Object.keys($changes).length > 0}
            <button class="btn primary" on:click={() => confirmModal = true}>
                Save
                <div class="btn-notification">{changesCount}</div>
            </button>
